...
- **ENV-HOSTED** When `BOARDVIZ_HOSTED` is set (non-empty, not `0`) — or the app is launched with a `--hosted` argument (`streamlit run … -- --hosted`), which is the local way to preview the demo — the app runs as a read-only hosted demo: no data processing happens in the cloud, and the Dashboard becomes the landing page. The Import page stays **visible but inert** — its actions (fetch, analyze) are disabled and a notice explains that importing isn't available in the hosted demo and points at the local install — so visitors can see the capability they'd get by installing. Visitors explore and drill the shipped sample.
